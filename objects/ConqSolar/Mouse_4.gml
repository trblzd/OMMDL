if (global.pintor == 5 and sprite_index = pintorcolorido) { // Verifica se a conquista está desbloqueada
    var file_path;

    // Define o caminho para salvar baseado na plataforma
    if (os_type == os_windows) {
        file_path = get_save_filename("*.png", "ConqSolar.png"); // Usuário escolhe onde salvar
    } else if (os_type == os_android || os_type == os_ios) {
        file_path = working_directory + "ConqSolar.png"; // Caminho padrão em dispositivos móveis
    }

    if (file_path != "") { // Verifica se o usuário escolheu um caminho válido
        var source_file = "datafiles/ConqSolar.png"; // Caminho do arquivo dentro do projeto
